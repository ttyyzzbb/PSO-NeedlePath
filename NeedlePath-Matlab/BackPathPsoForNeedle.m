function [arcsLength, Pgv,sump]=PsoForNeedle(arcsLengthUpperBound, arcsLengthLowerBound, evaluationFun, P0, pic, O0, Ptarget, radius)
%粒子群优化算法
...
Pv(:) = 10^10;
Pgv(:) = 10^10;
[P, Pv, Pg, Pgv,sump, sumPgv] = refreshBestStates(V, Z, M, evaluationFun, P0, pic, O0, Ptarget, radius, P, Pv, Pg, Pgv);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%开始迭代优化粒子群算法
time=[];%存储迭代次数
for i = 1:N
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
function [P, Pv, Pg, Pgv,sump, sumPgv] = refreshBestStates(V, Z, M, evaluationFun, P0, pic, O0, Ptarget, radius, P, Pv, Pg, Pgv)
...
%     Kerror = 30; %原始值为30
    ...
    for i = 1:M %所有粒子迭代循环
        [pathLen, tipError, deviationValue, obstacle, len] = evaluationFun(P0, pic, O0, Ptarget, Z(i,:), radius);
    ...
    for lunpan_num = 1:M
        ...
        [pathLen, tipError, deviationValue, obstacle, len] = evaluationFun(P0, pic, O0, Ptarget, Z(Select,:), radius);
        if len < particalLength
            Z(i,len+1:particalLength) = 0;
        end
        sump = pathLen * Klen + tipError * Kerror + deviationValue * Klen + obstacle; % 这一代粒子的适应值
